<template>
  <div id="faq" class="py-20 bg-gradient-to-bl from-[#fff6e5] to-[#ffdead]">
    <div class="px-4 sm:px-16">
      <div class="container mx-auto">
        <div class="text-44 md:text-66 font-semibold">{{ t("home.faq.title") }}</div>
        <div class="grid grid-cols-3 gap-4">
          <div class="col-span-3 desktop:col-span-1  text-30 px-4 md:pl-0">
          ...
          <div class="col-span-3 desktop:col-span-2">
            ...
            <div v-if="isShowMore">
              <div @click="clickActive(index)" :tabindex="index" v-for="(item, index) in itemList" :key="index"
                ...
                class="collapse collapse-plus  border-b rounded-none border-base-300 bg-base-200 border-y-current">
                <div class="collapse-title font-medium text-30 md:text-36">
                  {{ item.question }}
                </div>
                <div class="collapse-content text-24 text-a-gray-696F6F" v-html="item.answer">
                </div>
              </div>
            </div>
            <div v-else>
              <div @click="clickActive(index)" :tabindex="index" v-for="(item, index) in itemLimit" :key="index"
                ...
                <div class="collapse-content text-24 font-light" v-html="item.answer">
                </div>
              </div>
            </div>
...
export default defineComponent({
  ...
  setup(props, ctx) {
    ...
    const initialData = (data) => {
      if (props.lang) {
        itemList.value = props.lang === 'en' ? data.en : data.th;
