<template>
  <div class="container mx-auto mt-10">
    ...
    <div v-if="!isShowMore"   class="relative items-center flex justify-center">
      <div
        class="bg-a-black-121212 opacity-90 flex justify-center items-center absolute right-0 w-8 shadow rounded-l-md h-40 z-[4]">
        <div class="">
          <ChevronRightIcon class="h-8 w-8 text-white"></ChevronRightIcon>
        </div>
      </div>
    <div class="slides overflow-scroll smooth-scroll w-full touch-pan-x snap-mandatory flex snap-x py-10 gap-4">
    <div
        class="carousel-item  gap-4 pl-4"
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
        <div v-for="(item, index) in itemList" :key="index">
          <Card :badge="item.acf.badge" :cardTitle="item.title.rendered" :description="item.acf.description"
            :image="item.acf.thumbnail" :customClass="'w-full'" @click="clickShow(item)">
          </Card>
    ...
    <ModalMaster v-if="showModal" @close="clikeClose" :customClass="'content-center'" :isShow="showModal">
      ...
      <div class="parent relative w-full rounded-lg bg-white text-a-black-121212 fac-content">
        <SwiperGallery :items="itemShow.gallery" :customHeight="'h-[250px] md:h-[350px] rounded-t-lg'">
        </SwiperGallery>
        <div class="div-content text-left max-h-[21rem] overflow-y-auto">
...
import SwiperEtc from "../../components/SlideShow/SwiperEtc.vue"
