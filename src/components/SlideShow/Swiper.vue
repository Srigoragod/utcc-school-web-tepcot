    ...
    <div class="container mx-auto">
      ...
      <swiper v-else :spaceBetween="30" :style="{
        ...
        disableOnInteraction: false
          ...
          clickable: true,
        }" :lazy="true" :modules="modules" class="mySwiper drop-shadow shadow-2xl shadow-blue-500/20">
        <swiper-slide v-for="(item, index) in sliderList" :key="index">
          <div class="bg-image mx-auto is-desktop  rounded-lg  relative px-4"
            ...
            loading="lazy">
            <div  class="absolute z-4 bottom-0 inset-0 bg-gradient-to-t from-[#030e62]/50 via-[#030e62]/20 to-transparent h-full mx-auto rounded-b-lg pointer-events-none"></div>
            <div class="hero-content absolute inset-0 flex items-center justify-center text-center px-6 z-10 mx-auto pt-20">
              ...
              <button v-if="item.button1?.link" class="inline-block px-6 py-3 btn btn-accent text-white rounded-full text-24 font-light" @click="clickSlide(item.button1.link)"> 
                 {{ item.button1.title ? item.button1.title : 'ดูรายละเอียดเพิ่มเติม' }}
              </button>
              <button v-if="item.button2?.link" class="inline-block px-6 py-3 btn btn-neutral rounded-full text-white text-24 font-light" @click="clickSlide(item.button2.link)">
