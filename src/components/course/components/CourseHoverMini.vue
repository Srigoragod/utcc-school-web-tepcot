<template>
  <!-- container -->
  <a :href="`/${item.slug}`" class="custom-link cursor-pointer">
    <CardCourseMiniV2 :item="item" :lang="lang"></CardCourseMiniV2>
  </a>
...
import CardCourseMiniV2 from "../../card/CardCourseMiniV2.vue";
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const clickShow = (slug) => {
      window.origin.replace(`${slug}`)
    }
...
.bg-overlay {
  ...
  background: linear-gradient(0deg, #000000 0%, rgba(11, 11, 11, 0) 30%);
}
